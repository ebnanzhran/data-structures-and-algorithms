...
const sortByChildren = (charArray) => {
  // Solution code here...
  return charArray.sort((cur,prev) => {
    if (cur.children.length > prev.children.length) {
      return 1;
    }
    if (cur.children.length < prev.children.length) {
      return -1;
...
const containsWorld = (input) => {
  // Solution code here...
  let isContain = input.includes('world')
  return isContain;
};
