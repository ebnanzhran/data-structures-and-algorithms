...
function transformToLis(obj){
  // Solution code here...
  let newArr = [];
  Object.entries(obj).forEach(([x, y]) => newArr.push(`<li>${x}: ${y}</li>`));
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  let a = [];
  data.map(obj => {
    return obj.gender === 'male' || obj.gender === 'female' ? a.push(obj.name) : 0;
  });
  return a.join(' and ');
...
let findShortest = (data) => {
  ...
  let result = data.map( (character) => {
    character.height > cur ? cur = character.name : '';
  });
  return cur;
