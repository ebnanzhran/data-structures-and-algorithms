...
const addPurchases = (arr) => {
  ...
  const sumWithInitial = arr.reduce(
    (previousValue, currentValue) =>
     previousValue + currentValue.purchasePrice,initialValue);
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const num=0;
  const sum=arr.reduce((previousValue,currentValue,currentIndex)=>currentIndex)
...
const returnNames = (arr) => {
  // Solution code here...
  let arr2=[]
  arr2.push(arr[0].name)
    const sol=arr.reduce((previousValue,currentValue,currentIndex)=>
    
    arr2.push(arr[currentIndex].name)
...
const reversedString = (str) => {
  ...
  const sol=init.reduce((prev,curr)=>
  curr+prev);
  return sol
};
